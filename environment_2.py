...
DESCRIPTION : IN THIS ENVIRONMENT WHENEVER THE ROBOT HITS THE BALL, THE BALL APPEARS IN A NEW RANDOM POSITION
RUNNING THSI SCRIPT THIS GAME CAN BE PLAYED USING ARROW KEYS FROM KEYBOARD
...
while not gameExit :
    ...
    if ball_distance < 64 and angle_ball != -1 :
        raw_ball_x_position = random.randint(73 , 427)
        ball_x_position = round(raw_ball_x_position)
        last_ball_x_position = ball_x_position
        raw_ball_y_position = random.randint(73 , 427)
...
########################       calculation of front distance
    ...
    angle_between_ball = round( - (math.atan2((raw_ball_y_position - raw_y_position) , (raw_ball_x_position - raw_x_position))* 180 / pi + 90) + 360 ) % 360
    distance_ball = math.sqrt((raw_y_position - raw_ball_y_position) ** 2 + (raw_x_position - raw_ball_x_position) ** 2)
    if angle_between_ball >= face_angle - 15 and angle_between_ball <= face_angle + 15 :
        dist_ball = distance_ball
        angle_ball = angle_between_ball - (face_angle - 15)
    else :
        dist_ball = -1
        angle_ball = -1
